...
use App\Traits\Providers\NexosPlayTrait;
...
{
    use KaGamingTrait, FiversTrait, VibraTrait, SalsaGamesTrait, WorldSlotTrait, Games2ApiTrait, NexosPlayTrait;
    ...
    public function index()
    {
        $providers = Provider::where('status', 1)
            ->whereHas('games')
            ->whereIn('code', ['PGSOFTGAMES', 'PRAGMATICPLAY'])
            ->orderBy('code', 'asc')
            ->get();

        foreach($providers as $provider) {
            $provider->setRelation('games', $provider->games()->where('status', 1)->take(24)->get());
        }

        // Filter out providers with no games
        $providers = $providers->filter(function($provider) {
            return $provider->games->count() > 0;
        })->values();
            ...
            if(auth('api')->check()) {
                ...
                if($wallet->total_balance > 0) {
                    ...
                    switch ($game->distribution) {
                        ...
                        case 'nexosplay':
                            $nexosplayLaunch = self::GameLaunchNexosPlay($game->provider->code, $game->game_code, 'pt', auth('api')->id());

                            if(isset($nexosplayLaunch['launch_url'])) {
                                return response()->json([
                                    'game' => $game,
                                    'gameUrl' => $nexosplayLaunch['launch_url'],
                                    'token' => $token
                                ]);
                            }

                            return response()->json(['error' => $nexosplayLaunch, 'status' => false ], 400);
