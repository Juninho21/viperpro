<x-filament-panels::page>
    ...
    <div class="w-full p-4 bg-gray-500 shadow-lg mt-5" style="background-color: #51515163;border-radius: 5px;">
        <h2 class="mb-5 text-3xl">NexosPlay</h2>
        <hr style="border-color: #4b4b4b8c;padding-bottom: 10px">
        <div class="flex justify-between w-full gap-4">
            <div class="flex flex-col gap-4 w-full justify-between">
                <button wire:click="loadProviderNexosPlay('slot')" class="bg-primary-600 px-3 py-2 w-full text-center">
                    <div wire:loading wire:target="loadProviderNexosPlay('slot')">Carregando Provedores</div>
                    <div wire:loading.remove wire:target="loadProviderNexosPlay('slot')">Carregar Provedor (Slot)</div>
                </button>
                <button wire:click="loadProviderNexosPlay('casino')" class="bg-primary-600 px-3 py-2 w-full">
                    <div wire:loading wire:target="loadProviderNexosPlay('casino')">Carregando Provedores</div>
                    <div wire:loading.remove wire:target="loadProviderNexosPlay('casino')">Carregar Provedor (Casino)</div>
                </button>
                <button wire:click="loadProviderNexosPlay('pachinko')" class="bg-primary-600 px-3 py-2 w-full">
                    <div wire:loading wire:target="loadProviderNexosPlay('pachinko')">Carregando Provedores</div>
                    <div wire:loading.remove wire:target="loadProviderNexosPlay('pachinko')">Carregar Provedor (Pachinko)</div>
                </button>
            </div>
            <button wire:click="loadGamesNexosPlay" class="bg-primary-500 px-3 py-2 w-full">
                <div wire:loading wire:target="loadGamesNexosPlay">Carregando Jogos</div>
                <div wire:loading.remove wire:target="loadGamesNexosPlay">Carregar Jogos</div>
            </button>
        </div>
    </div>
